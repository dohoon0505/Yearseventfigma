...
import { Image as ImageIcon, X, Save, CheckCircle } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-[10px] shadow-2xl w-full max-w-[460px] mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e0e0e0]">
          <div>
            <p className="text-[12px] text-[#888] font-medium">{product.category}</p>
            <h3 className="text-[16px] text-[#222] font-bold">{product.product}</h3>
          </div>
        ...
        <div className="p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[14px] text-[#333] font-medium">상품금액</span>
            <span className="text-[18px] text-[#f15a2a] font-bold">{product.price}</span>
          </div>
          ...
          <div className="mt-4 bg-[#fffbf0] border border-[#f2e0a0] rounded-[4px] px-3 py-2">
            <p className="text-[12px] text-[#888]">※ 실제 상품은 사진과 다를 수 있으며, 계절 및 산지 사정에 따라 품종이 변경될 수 있습니다.</p>
          </div>
        </div>
...
export function ProductGuide() {
  ...
  const columns: Column<Product>[] = [
    {
      label: "즐겨찾기", width: "60px", align: "center",
    ...
    { label: "구분", width: "80px", render: (r) => r.category },
    { label: "상세상품", width: "160px", render: (r) => r.product },
    { label: "상품금액", width: "110px", align: "right", render: (r) => <span className="font-medium text-[#444]">{r.price}</span> },
    { label: "상품설명 및 비고(규격)", render: (r) => r.description },
    {
      label: "샘플사진", width: "80px", align: "center",
      render: (r) => (
        <button
          onClick={() => setSampleProduct(r)}
          className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-[#f5f5f5] hover:bg-[#ffe9e9] border border-[#e0e0e0] hover:border-[#f15a2a] rounded-[4px] text-[12px] text-[#555] hover:text-[#f15a2a] transition-colors whitespace-nowrap"
        >
          <ImageIcon size={13} />
          <span>보기</span>
        </button>
  ...
  return (
    <div className="p-6">
      <PageTitle icon="🌈" title="상품 규격 안내" />

      {/* Filters */}
      <div className="bg-white border border-[#e0e0e0] rounded-[6px] p-4 mb-4 flex flex-col gap-3">
        <div className="flex items-center gap-4">
          <span className="text-[13px] text-[#555] font-medium w-[80px]">상품조회구분</span>
          <div className="flex items-center gap-4">
            {categories.map((cat) => (
              <label key={cat} className="flex items-center gap-1.5 cursor-pointer">
                ...
                <span className={`text-[13px] ${selectedCategory === cat ? "text-[#f15a2a] font-medium" : "text-[#555]"}`}>{cat}</span>
              </label>
            ))}
          </div>
        </div>
        <div className="flex items-start gap-4">
          <span className="text-[13px] text-[#555] font-medium w-[80px] pt-0.5">즐겨찾기안내</span>
          <p className="text-[13px] text-[#666] leading-[1.6]">자주 이용하는 상품을 즐겨찾기에 선택해두면 경조상품 주문 시 상품 선택을 수월하게 할 수 있습니다.</p>
        </div>
      </div>
