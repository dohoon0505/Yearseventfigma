...
tools/test-*.mjs        # node 회귀 테스트 — 주문 문자 파서 39케이스 · 정산 규칙 114케이스
serve.mjs               # 개발용 정적 서버
```
...
- 백엔드·인증은 미구현(목데이터). 로그인은 DEMO 게이트입니다 — 거래처는 **아이디가 계정과 맞고 상태가 `활성`**일 때만
  들어갑니다(모르는 아이디·사업자번호는 거절, 승인대기·반려·정지는 상태별 안내). 비밀번호는 셀프 가입 계정만 검사합니다.
- 영속 데이터(거래처·담당자·발송 프로필·상품단가·즐겨찾기·**계산서 발급 동의**)는 `localStorage` 키
  **`yeop.store.v4`** 에 저장됩니다. 동의 기록은 거래처·귀속월별이고 되돌릴 수 없으므로, 데모 전에는
  저장소를 비우는 편이 좋습니다. 주문은 모듈 메모리라 새로고침하면 시드로 돌아갑니다.
