...
- **거래처 담당자 관리**는 `js/util/contacts-modal.js` 의 **다이얼로그**다(별도 탭이 아니다 — 2026-09-16 사용자 지시). 진입점 둘: 거래처 목록 '관리' 열의 **사람 아이콘**(`.tbl-users`, 등록 인원수를 함께 찍어 0명인 거래처가 보이게 한다)과 거래처 모달 레일의 '정산·회계 담당자' 카드. 모달 원장에서는 담당자 표를 걷어냈고(시안) 레일 카드는 **읽기 전용**이다.
  ...
  - **링크 자동작성**(`openAutofill`)은 시안대로 **소스 세그먼트 3종**(부고장 링크 · 청첩장 링크 · 텍스트 인식)이다. 링크 모드는 인식된 `kind` 가 고른 모드와 다르면 **닫지 않고** 어느 세그먼트로 바꾸면 되는지 지목한다. 텍스트 모드는 붙여 넣은 **문장 전체**를 그대로 `parseOrderUrl` 에 넘긴다 — 그 함수가 `includes` 판정이라 문장 속 링크를 그대로 찾는다(파서를 복제하지 말 것). 문장 해석(AI)은 아직 없으므로 **힌트 문구가 그 사실을 말해야 한다** — 시안 문구를 그대로 베끼면 못 하는 일을 약속하게 된다.
