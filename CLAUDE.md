...
- **거래처 담당자 관리**(`#/admin/contacts`)는 거래처 모달과 **별개 화면**이다. 담당자 표가 거기 산다 — 거래처 모달에서는 걷어냈고(시안), 레일의 '정산·회계 담당자' 카드는 **읽기 전용**이다. ⚠️ 관리자가 담당자를 고칠 경로를 없애면 안 된다 — 구 시스템에 담당자 필드가 없어 이관 거래처 19곳 중 **18곳이 담당자 0명**이고, 담당자는 주문 요청자·배송완료 알림 수신자·정산 명세서 수신자의 출처다. 정산담당 1명 불변식과 삭제 잠금은 포털과 **똑같이** 지킨다.
  - 좌측 거래처 피커는 **공용 `.cpick-*`**(components.css)다 — 기업별 상품단가와 같은 껍데기. 두 화면 다 "거래처를 고르고 그 거래처의 무언가를 고친다"라 고르는 방법이 달라선 안 된다. 페이지 고유 확장만 `.page-* .cpick-*` 로(예: `.page-pricing .cpick-shell` 의 하단 패널 여백).
